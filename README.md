...
```{warning}
这是一本编写中的书，现在只是草稿阶段。书名为《Agentgateway 内幕》，英文名《Agentgateway Insider》。
```
...
本书不是一本使用手册。更不是从使用者角度，教如何深入浅出学习使用 Agentgateway。不会布道 Agentgateway 有如何如何强大之功能，更不会教如何使用 Agentgateway。这方面网上已经有太多非常优秀的书、文章、文档了。
...
- Agentgateway 设计理念与实现架构。
- Agentgateway 主要功能实现流程。
- Agentgateway 如何利用 ansync Rust 以及 3rd-party 库 tokio / hyper 网络代理服务器。


书里说的，只是在我研究与使用了 Agentgateway 一段时间后，的思考与记录。我不是专家，更不是 Agentgateway Committer(不过我正在努力准备中)。连互联网大厂员工也不是。只是浏览和 Debug 过一些 Agentgateway/Agentgateway 的代码。

在研究 Agentgateway 过程中。发现网上是有很多非常有价值的资讯。但是，要么主要是从使用者出发，没说实现机理；要么就是说了机理，也说得很好，但内容少了系统化和连贯性。

### 读者对象
本书主要讲 Agentgateway/Agentgateway 的设计、实现机制。假设读者已经有一定的 Agentgateway 使用经验。
